		...
		<div class="card balance">
			<div class="card-content">
				<div class="card-title">
					<h5 class="subtitle has-smaller-margin is-5 has-text-weight-bold">Total balance</h5>
				...
				<h5 class="subtitle has-smaller-margin is-5 has-text-weight-bold mt-5">Network stats</h5>

				<div class="market-data">
					<div class="has-text-weight-normal">Total supply</div>
					<div
				...
				<div class="market-data">
					<div class="has-text-weight-normal">Market cap</div>
					<div class:skeleton={!$bitcoinMarketData.market_cap}>
						{labelAmountValue($bitcoinMarketData.market_cap, true, $selectedCurrency, 2)}<span class="is-size-7 ml-1 has-text-multiline-full-word"
							>{$selectedCurrency}</span
						>
					</div>
				</div>
				<div class="market-data">
					<div class="has-text-weight-normal">Satoshi{100000000 / $bitcoinCurrentPrices[$selectedCurrency] < 1 ? '' : 's'} per {$selectedCurrency}</div>
				...
				<div class="market-data">
					<!-- TODO: real status -->
					<div class="has-text-weight-normal">Status</div>
					...
					<div class="is-vertical-center has-text-right" title="Blocksteam Esplora API">
						...
						{#if navigatorOnline && $bitcoinNetworkBlockHeight >= 1 && totalCurrentBalance !== undefined && totalCurrentBalance !== -1}
							{$bitcoinTestnetNetwork ? 'Testnet' : 'Mainnet'} online
						{:else if !navigatorOnline}
							Offline
						{:else}
							Connecting to {$bitcoinTestnetNetwork ? 'Testnet' : 'Mainnet'}
						{/if}
					</div>
				</div>

				<div class="market-data">
					<div class="has-text-weight-normal">Block height</div>
				...
				<div class="card-bottom pt-3">
					<div class="buttons is-centered mt-6-custom">
						<Button
							...
							buttonDisabled={true}
							title="Subscribe to our newsletter for updates or follow us on twitter"
						/>
					</div>
				</div>
			</div>
					...
					<div class="card-content">
						<div class="card-title">
							...
							<h3 class="subtitle is-6 has-text-weight-normal is-family-primary is-vertical-center">
								<span class="icon is-normal is-prussian-blue has-no-hover mr-2 mb-1"><img src={vaultIcon} alt="Vault icon" /></span>{quorum.requiredSigners} of {quorum.totalSigners}
								Vault
							</h3>
						</div>
						...
						<div class="buttons is-right">
							<Button text="View details" icon={'arrowRight'} buttonClass="is-primary" on:buttonClicked={() => handleChangeCurrentConfigFile(id)} />
						</div>
					</div>
				</div>
			</div>
					...
					<div class="card-content">
						<div class="card-title">
							...
							<h3 class="subtitle is-6 has-text-weight-normal is-family-primary is-vertical-center">
								...
								>{$configsCurrentDataVaultsArray[i].config.quorum.requiredSigners} of
								{$configsCurrentDataVaultsArray[i].config.quorum.totalSigners}
								Vault
							</h3>
						</div>
						<h5 class="subtitle has-smaller-margin is-5 has-text-weight-bold">Balance</h5>
						...
						>
							{$userSettings.satoshiUnit
								...
								class="is-size-7 ml-1">{$bitcoinTestnetNetwork ? 't' : ''}{$userSettings.satoshiUnit ? 'sats' : 'BTC'}</span
							>
						</p>
